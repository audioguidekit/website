...
| `imageColor` | `string` | Background color of the TourStart screen and its status bar area (used when there's no cover image) |
...
Add `imageColor` to your `metadata.json` to control the TourStart status bar:
