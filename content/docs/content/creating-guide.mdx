...
| `mapMarkerCustomIcon` | No | URL to a custom marker image used for all stops |
| `mapMarkerNumber` | No | Show stop number on markers (default: `true`) |
...
| `mapMarkerIcon` | No | Custom marker image URL for this stop only (overrides tour-level `mapMarkerCustomIcon`) |
