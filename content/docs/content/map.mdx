...
|----------|-------------|---------------------|-------|
| OpenFreeMap | No | `"openfreemap"` (default) | Vector |
| OpenStreetMap | No | `"openstreetmap"` | Raster |
| CARTO | Yes | `"carto"` | Raster |
| Mapbox | Yes | `"mapbox"` | Raster |
| Jawg Maps | Yes | `"jawg"` | Raster |
| MapTiler | Yes | `"maptiler"` | Raster |
...
Or simply omit `mapProvider` — this is the default.
...
Requires an API key — CARTO retired its free tier. Without a key, `mapProvider: "carto"` silently falls back to OpenStreetMap. Four clean styles to choose from:
