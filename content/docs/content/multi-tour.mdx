...
A branding option for museums: a **full-screen image or video** shown over the selection screen until the visitor taps to continue. The selection screen is functional and hard to brand; the splash gives you a clean, full-bleed canvas first.
...
- **Single tour** → the selection screen is skipped and the tour opens directly, so `title`, `subtitle`, `logo`, `hero` and `tourCard` never render. `splash` is the exception: it plays over the tour on first load too — which also means it appears over a deep link straight into a tour (`/tour/<id>`) in multi-tour apps.
- **Themes stay in code.** `app.json` references a registered `themeId`; it cannot define a new theme inline. To add a new look, create a theme in `src/theme/themes/` — see [Custom themes](/docs/branding/custom).
- **No per-tour override for `tourCard`** — card layout is intentionally uniform across the list.
- A **"Back to tours"** button appears on each tour's start screen (multi-tour deployments only), returning the visitor to the picker with a native iOS-style slide.

<Warning>
**Rebuild after editing `app.json`.** It is bundled into the app at build time (via `import.meta.glob`), so changes require a dev-server reload or a rebuild to take effect.
</Warning>
