...
**Root cause / approach:** Bumped MapTiler's default style from `outdoor-v2` to `outdoor-v4` (MapTiler's current generation) in both the docs and the actual code default (`player-react/src/utils/mapTileProvider.ts`), since documenting a default that didn't match the code would just move the bug from the page to the tile requests.

→ *No new memory entries — [[docs-sync-from-player-react]] already covers the field-verification workflow this used.*
